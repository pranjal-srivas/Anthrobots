...
```{r}
mypcadat <- read_csv("/Users/pranjal/Desktop/Programming/Gizem Custom Tools/Cilia Coverage/AllMov3.csv")
mypcadatA <- read_csv("/Users/pranjal/Desktop/Programming/Gizem Custom Tools/Cilia Coverage/AllMov4.csv")
```

Append and clean up data from the two files. 

```{r}
mypcadatB <- filter(mypcadatA, Keep == TRUE)#& Category != "Nonmover")# & Category == "Mover")
mypcadatB <- mypcadatB[,-19]# & Category != "Nonmover")# & Category == "Mover")[,-19]
mypcadatB <- mypcadatB[complete.cases(mypcadatB), ]
mypcadat2 <- filter(mypcadat, Keep == TRUE)#& Category != "Nonmover")# & Category == "Mover")
mypcadat2 <- rbind(mypcadat2, mypcadatB)#[1:350,]#
nrow(mypcadat2)
...
finalgrid <- pivot_longer(rawdatfin0, colnam, "Key")
```
