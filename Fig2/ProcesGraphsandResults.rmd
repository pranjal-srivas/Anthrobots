...
mm of the arena wall in order to eliminate edge effects on the bots' behaviors. We also filter out any chunks beyond 5 hours (chunk size > 600).
...
activity_summ <- tracks %>%
  ...
  dplyr::summarize(., norm_error = sum(error, na.rm = TRUE) / sum(lin_dist, na.rm = TRUE),
            n = n(), 
            discard = any(arena_dist > cutoff_dist),
            .groups = "drop") %>% #, radius = circlefit(x,y)[3]
  ungroup(.) %>%
  filter(., n == 12)
keydenom <- nrow(activity_summ)
nrow(filter(activity_summ, group>600))/keydenom
activity_summ <- filter(activity_summ, group < 600)
activity_summ
nrow(filter(activity_summ, discard == TRUE))/keydenom
...
gmix <- activity_summ %>%
  ...
  list(df = .) %>%
  REBMIX(Dataset = ., pdf = "gamma", Preprocessing = "histogram",cmin = 2, cmax = 3,Criterion = "BIC")# 
...
activity_class <- activity_summ %>%
  ...
  mutate(., activity = if_else(cl == which.max(mus) | norm_error > max(mus), 
                               "inactive", "active")) %>%
  dplyr::select(., id, group, activity)#, radius
```
...
```{r message=FALSE}
library(circular)
nrow(dplyr::filter(activity_class, activity == "inactive"))/keydenom
...
g1 <- ggplot(behavior_summ, aes(x = straightness)) +
  ...
  labs(x = "Straightness index",
       y = "Density") +
  theme_light(base_size = 62)#, base_family = "Raleway")
...
g3 <- ggplot(behavior_summ, aes(x = straightness, y = gyration)) +
  ...
  geom_point(aes(size = 2.5), alpha = 0.1, size = 10) +
  geom_density_2d(alpha = 0.2, color = "black") +
  labs(x = "Straightness index", 
       y = "Gyration index") +
...
behavior_summ <- behavior_summ[complete.cases(behavior_summ), ]
...
clusters <- behavior_summ %>%
  ...
  as.matrix(.) %>%
  cec(., init, iter.max = 1000, card.min = "5%")
  
#This part reassigns cluster numbers
clusters$cluster <- ifelse(clusters$cluster == 3, 2, clusters$cluster)
clusters$cluster <- ifelse(clusters$cluster == 5, 1, clusters$cluster)
clusters$cluster <- ifelse(clusters$cluster == 4, 3, clusters$cluster)
clusters$cluster <- ifelse(clusters$cluster == 6, 4, clusters$cluster)
...
behavior_class <- behavior_class %>%
  full_join(., class_stats %>% dplyr::select(., class, behavior), by = "class")
...
datdat <- as.data.frame(clusters$data)
datdat1 <- cov(filter(datdat, clusters$cluster == 1))
datdat2 <- cov(filter(datdat, clusters$cluster == 2))
datdat3 <- cov(filter(datdat, clusters$cluster == 3))
datdat4 <- cov(filter(datdat, clusters$cluster == 4))
datdat0 <- datdat
datdat0$cluster <- clusters$cluster
datdat0 <- datdat0 %>% dplyr::group_by(cluster) %>% dplyr::summarise(straightness = mean(straightness), gyration = mean(gyration))
lis <- list(datdat1,datdat2,datdat3,datdat4)

ellipses <- t(mapply(cov2shape, lis, asplit(clusters$centers[1:4,], 1), #clusters$covariances[1:4] #clusters$centers[1:4,]#as.matrix(datdat0)[,2:3]
                     SIMPLIFY = TRUE)) %>%
  as_tibble(.) %>%
...
The final classification resulted in four categories, two of them (1 and 2) 
with high internal consistency (i.e., low average dissimilarity) and the other 
two (3 and 4) being more diffuse. Two of the five categories (1 and 2) represent 
more than 60% of the active chunks, indicating that the bots' most common 
behavior is to perform circling or linear-like movements (categories 1 and 2 have the highest 
gyration and straightness indices of the four behavioral categories). 
...
final_behavior_summ <- filter(final_behavior_summ, final_behavior_summ$behavior != "Inactive")
my_comparisons2 <- list(c("1", "2"), c("1", "3"), c("1", "4"), c("2", "3"), c("2", "4"), c("3", "4"))
...
g1 <- ggplot(final_behavior_summ, aes(x = factor(behavior), y = straightness, 
  ...
  scale_fill_manual(values = c(brewer.pal(8, "Dark2"), "#333333")) +
  theme_light(base_size = 62)+#, base_family = "Raleway") +
  theme(legend.position = "none")+stat_compare_means(aes(label = ..p.signif..), method = "wilcox.test", vjust = 0.8, hjust = 1.1, comparisons = my_comparisons2, p.adjust.method = "holm", size = 14, bracket.size = 3)
...
tracks_sample <- tracks_cl %>%
  ...
  ungroup(.)
#write.csv(tracks_sample, "tracks_sample.csv")
...
g <- tracks_sample %>%
  ...
  do(., g = ggplot(., aes(x, y)) +
       ...
       geom_text(aes(label = paste0(id,"_",group)))+
...
gg <- tracks_sample %>%
  ...
  ungroup(.) %>%
  group_by(., behavior) %>%
  do(., gg = wrap_plots(.$gg, nrow = 3) +
       plot_annotation(title = paste0("Category ", .$behavior[1]),
                       theme = theme(plot.title = element_text(size = 16)))) %>%
  ungroup(.) 

wrap_elements(gg$gg[[1]]) + wrap_elements(gg$gg[[2]]) + wrap_elements(gg$gg[[3]]) +
      wrap_elements(gg$gg[[4]]) 
gg


g2 <- filter(tracks_sample, tracks_sample$behavior != 0) %>%
  group_by(., behavior, id, group) %>%
  do(., g2 = ggplot(., aes(x, y)) +
       geom_path() +
       #coord_fixed(xlim = range(lims), ylim = range(lims)) +
       #coord_fixed(xlim = range(c(.$x, .$y)), ylim = range(c(.$x, .$y))) +
       scale_x_continuous(breaks = seq(-0.3, 0.3, 0.01)) +
       scale_y_continuous(breaks = seq(-0.3, 0.3, 0.01)) +
       theme_light(base_size = 16)+#, base_family = "Raleway")  +
  ...
  ungroup(.) %>%
  group_by(., behavior) %>%
  do(., g2 = wrap_plots(.$g2, nrow = 3) +
       plot_annotation(title = paste0("Class ", .$behavior[1]),
                       theme = theme(plot.title = element_text(size = 20)))) %>%
  ungroup(.) 

invisible(lapply(g2$g2, print))
```
...
followed by a chunk of the same or another category. 
...
final_behavior_summ <- final_behavior_summ %>%
  mutate(., behavior_simplified = if_else(behavior == "1", "1",
                                          if_else(behavior == "2", "2",
                                                  if_else(behavior == "3", "3", behavior))))
...
The estimated transition network is represented below. 
...
m<- graph_table %>% 
  ...
  geom_edge_fan(aes(label = percent(weight, accuracy = 0.01, digits = 3), edge_width = sqrt(sqrt(weight))), 
                arrow = grid::arrow(length = unit(8, "mm")),
                end_cap = circle(35, 'mm'),
                start_cap = circle(15, "mm"),
                angle_calc = "along",
                label_dodge = unit(-7, "mm"),
                label_size = 10) + 
  geom_edge_loop(aes(label = percent(weight, accuracy = 0.01, digits = 3),
                     edge_alpha = weight, span = 30, edge_width = (sqrt(sqrt(weight))),
                     direction = c(45, 0, 0, 0, 0, -45, 0, 0, 0, 0, -135, 0, 0,0,0,135)),
                 arrow = grid::arrow(length = unit(8, "mm")),
                 start_cap = circle(15, "mm"),
                 ...
                 label_dodge = unit(-7, "mm"),
                 label_size = 10) +
  geom_node_point(size = 80) +
  geom_node_text(aes(label = paste0(name, "\n", percent(count/tot,accuracy = 0.01, digits = 3))), color = "white", family = "sans", size =10) +
  scale_x_continuous(expand = expansion(c(.12, .12))) +
  scale_y_continuous(expand = expansion(c(.12, .12))) + 
  theme(legend.position = "none",
        panel.background = element_rect(fill = "white"))

trans_table <- as.data.frame(summary(graph_table[-7]))
trans_table
m
df <- as.data.frame(as.matrix(graph_table[-7]))
df
