...
for (i in 1:length(dat)) {
  scar <- Rvision::zeros(dat[[i]]$dims[1], dat[[i]]$dims[2], nchan = 1)
  ...
  for (j in 1:length(dat[[i]]$bot)) {
    if (!is.null(dat[[i]]$bot[[j]]$contour)) {
      ...
      tmp <- Rvision::zeros(dat[[i]]$dims[1], dat[[i]]$dims[2], nchan = 1)
...
## . Rotational tendency vs coverage (Fig 5C)

```{r}
g <- ggplot(filter(summsumm)) +
  aes(rt_up1, cov, ymin = cov_lo, ymax = cov_up, xmin = rt_up1, xmax = rt_up1, 
  ...
  geom_smooth(method = MASS::rlm, formula = y ~ x) +
  xlim(0, 1)+ ylim(0, 1) +
  labs(x = "Rotational tendency Upper Bound", 
       y = "Proportion of bot on tissue") +
  theme_light(base_size = 16) 
g
```
